...
def create_product(body, headers):
    try:
        ...
        uploadUrls = genPresignUrl(body.get('images'), product_id)
        ...
        return build_response(201, {'message': 'Product created successfully', 'product_id': product_id, 'uploadUrls': uploadUrls}, headers)
    except ClientError as e:
        return build_response(500, {'error': e.response['Error']['Message']}, headers)
...
def genPresignUrl(images, product_id):
    uploadUrls = []
    for image in images:
        key = f"products/{product_id}/{image['name']}"
        ...
        presigned_url = s3.generate_presigned_url(
            ...
            Params={
                "Bucket": bucket_name,
                "Key": key,
                "ContentType": image['type'] or "application/octet-stream",
            },
            ExpiresIn=900
        )
