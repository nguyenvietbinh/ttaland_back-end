...
def get_user(user_id, headers):
    try:
        ...
        if not user:
            return response(404, {'error': 'User not found'}, headers)
        
        return response(200, user, headers)
    except ClientError as e:
        return response(500, {'error': e.response['Error']['Message']}, headers)
...
def create_user(body, headers):
    try:
        ...
        if not body.get('id') or not body.get('name') or not body.get('email'):
            return response(400, {'error': 'Missing required fields: id, name, email'}, headers)
        
        users_table.put_item(Item=body)
        return response(201, {'message': 'User created successfully', 'user': body}, headers)
    except ClientError as e:
        return response(500, {'error': e.response['Error']['Message']}, headers)
...
def update_user(user_id, body, headers):
    try:
        ...
        return response(200, {'message': 'User updated successfully'}, headers)
    except ClientError as e:
        return response(500, {'error': e.response['Error']['Message']}, headers)
...
def delete_user(user_id, headers):
    try:
        ...
        return response(200, {'message': 'User deleted successfully'}, headers)
    except ClientError as e:
        return response(500, {'error': e.response['Error']['Message']}, headers)
